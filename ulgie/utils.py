...
def translate(img, translation):
    batch_size, num_slot, num_channel, height, width = img.shape
    translation[:, :, 0] = translation[:, :, 0] * img.shape[3]
    translation[:, :, 1] = translation[:, :, 1] * img.shape[4]
    translation = translation.type(torch.IntTensor)
    img = img.reshape(-1, num_channel, height, width)
    translation = translation.reshape(-1, 2)
    img = torch.cat(
        [torch.roll(img[i], tuple(translation[i].tolist()), (-2, -1)) for i in range(batch_size * num_slot)],
        dim=0
    )
    img = img.reshape(batch_size, num_slot, num_channel, height, width)
    return img
